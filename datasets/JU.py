...
#单个样本长度为1024
signal_size = 1024
root='E:\PythonProject\DataSets\JU_DataSet'
#0、1、2分别代表四种不同的工况     dataname是一个字典，其中的每个键对应着一个包含文件名的列表
dataname= {0:["_n600_3_2.mat","_ib600_2.mat", "_ob600_2.mat", "_tb600_2.mat"],  # 600rpm
           1:["_n800_3_2.mat","_ib800_2.mat", "_ob800_2.mat", "_tb800_2.mat"],  # 800rpm
           2:["_n1000_3_2.mat","_ib1000_2.mat", "_ob1000_2.mat", "_tb1000_2.mat"],  # 1000rpm
}
"""上述data对应的label为
    正常数据、内圈、外圈、0.014球、滚动体
    0、1、2、3
    共4个label
"""


label = [i for i in range(0, 4)]   #[0, 1, 2, 3]
...
def get_files(root, N):#N为转速，N传进来是一个代表负载
    ...
    for k in range(len(N)): #k只取一个值 0
        ...
        for n in tqdm(range(len(dataname[N[k]]))):  #这里迭代4次 取出4个.mat文件 如果n==0 取第一列normal数据，反之取12k的数据 长度是10
            #根据n的取值决定数据集的路径
            if n==0:
                #获得的是正常基线数据 字典中第一列的文件
               path1 =os.path.join(root,dataname[N[k]][n])  #/Users/chensheng/Desktop/JU_DataSet/
            else:
